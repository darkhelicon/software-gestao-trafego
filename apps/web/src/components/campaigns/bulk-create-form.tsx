...
const selectClass =
  "h-10 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-sm text-gray-300 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-400/20 transition-colors";
...
export function BulkCreateForm({ platform, onJobCreated }: BulkCreateFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!names.length) { setError("Adicione pelo menos um nome de campanha."); return; }
    if (names.length > 50) { setError("Máximo de 50 campanhas por vez."); return; }
    if (!advertiserAccountId) { setError("Selecione uma conta."); return; }

    try {
      if (selectedTemplateId) {
        const result = await applyTemplate.mutateAsync({ templateId: selectedTemplateId, advertiserAccountId, names });
      ...
      setError(err instanceof Error ? err.message : "Erro ao criar campanhas.");
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="flex flex-col gap-1.5">
        ...
        <select value={advertiserAccountId} onChange={(e) => setAdvertiserAccountId(e.target.value)} className={selectClass} required>
        ...
        {!accounts?.length && (
          <p className="text-xs text-brand-400">Nenhuma conta sincronizada. Sincronize na aba de conexões.</p>
        )}
      </div>
      ...
      {templates && templates.length > 0 && (
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-gray-300">
            Template <span className="text-gray-600 font-normal">(opcional)</span>
          </label>
          ...
          {selectedTemplateId && (
            <p className="text-xs text-brand-400">Config preenchida automaticamente pelo template.</p>
          )}
        </div>
      )}

      <div className="rounded-lg border border-white/5 bg-white/[0.02] p-4 flex flex-col gap-3">
        <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Configuração das campanhas</p>
          ...
          <>
            <div className="flex gap-3">
              <div className="flex flex-col gap-1.5 flex-1">
                ...
                <select value={tikObjective} onChange={(e) => setTikObjective(e.target.value)} className={selectClass}>
                  {TIKTOK_OBJECTIVES.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-sm font-medium text-gray-300">Orçamento</label>
                <select value={tikBudgetMode} onChange={(e) => setTikBudgetMode(e.target.value)} className={selectClass}>
                  {TIKTOK_BUDGET_MODES.map((b) => <option key={b.value} value={b.value}>{b.label}</option>)}
                </select>
              </div>
            </div>
            {tikBudgetMode !== "BUDGET_MODE_INFINITE" && (
              <Input id="bulk-tik-budget" type="number" label="Valor do orçamento (R$)" placeholder="Ex: 100" value={tikBudget} onChange={(e) => setTikBudget(e.target.value)} min="1" step="0.01" />
            )}
          </>
        ) : (
          <>
            <div className="flex gap-3">
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-sm font-medium text-gray-300">Objetivo</label>
                <select value={metaObjective} onChange={(e) => setMetaObjective(e.target.value)} className={selectClass}>
              ...
              <div className="flex flex-col gap-1.5 flex-1">
                ...
                <select value={metaBudgetType} onChange={(e) => setMetaBudgetType(e.target.value as "daily" | "lifetime")} className={selectClass}>
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                <Input id="bulk-meta-budget" type="number" label="Valor do orçamento (R$)" placeholder="Ex: 50" value={metaBudget} onChange={(e) => setMetaBudget(e.target.value)} min="1" step="0.01" />
              </div>
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-sm font-medium text-gray-300">Status inicial</label>
                <select value={metaStatus} onChange={(e) => setMetaStatus(e.target.value as "ACTIVE" | "PAUSED")} className={selectClass}>
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-gray-300">Nomes das campanhas</label>
          {nameCount > 0 && (
            <span className="text-xs text-brand-400 font-medium">{nameCount} campanha{nameCount !== 1 ? "s" : ""}</span>
          )}
        </div>
        <textarea
          value={namesInput}
          ...
          rows={6}
          className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-gray-600 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-400/20 resize-none font-mono transition-colors"
          required
        />
        <p className="text-xs text-gray-600">Um nome por linha · Máximo de 50 campanhas por envio</p>
      </div>
      ...
      <Button type="submit" isLoading={isPending} className="w-full" size="lg" disabled={!nameCount || !advertiserAccountId}>
        {nameCount > 0 ? `Criar ${nameCount} campanha${nameCount !== 1 ? "s" : ""}` : "Criar campanhas"}
      </Button>
    </form>
  );
}
