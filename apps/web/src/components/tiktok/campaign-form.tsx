...
export function CampaignForm({ onJobCreated }: CampaignFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.advertiserAccountId) {
      setError("Selecione uma conta advertiser.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        ...
        <select
          value={form.advertiserAccountId}
          onChange={setField("advertiserAccountId")}
        ...
        {!accounts?.length && (
          <p className="text-xs text-yellow-600">
            Nenhuma conta sincronizada. Sincronize na aba de conexões primeiro.
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select
          ...
          onChange={setField("objectiveType")}
          className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
        >
          {OBJECTIVES.map((o) => (
