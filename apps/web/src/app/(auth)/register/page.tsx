"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import {
  auth,
  createUserWithEmailAndPassword,
  signInWithPopup,
  googleProvider,
} from "@/lib/firebase";
import { api } from "@/lib/api";
import { useAuthStore } from "@/store/auth";
import type { OrgContext } from "@/store/auth";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

type MeResponse = {
  organizationUsers: Array<{
    role: OrgContext["role"];
    organization: {
      id: string;
      name: string;
      slug: string;
      subscription: OrgContext["subscription"];
    };
  }>;
};

export default function RegisterPage() {
  const setCurrentOrg = useAuthStore((s) => s.setCurrentOrg);
  const setFirebaseUser = useAuthStore((s) => s.setFirebaseUser);

  const [form, setForm] = useState({
    name: "",
    organizationName: "",
    email: "",
    password: "",
  });
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  // True when user is already signed in with Google (came from login page)
  const [googleSession, setGoogleSession] = useState(false);

  useEffect(() => {
    // Detect if a Google Firebase session already exists.
    // This happens when a new Google user is redirected here from the login page —
    // they already completed Firebase OAuth but aren't registered in our DB yet.
    if (!auth?.currentUser) return;
    const isGoogle = auth.currentUser.providerData.some(
      (p) => p.providerId === "google.com"
    );
    if (isGoogle) {
      setGoogleSession(true);
      setForm((prev) => ({
        ...prev,
        name: auth.currentUser?.displayName ?? "",
        email: auth.currentUser?.email ?? "",
      }));
    }
  }, []);

  function setField(field: keyof typeof form) {
    return (e: React.ChangeEvent<HTMLInputElement>) =>
      setForm((prev) => ({ ...prev, [field]: e.target.value }));
  }

  // Core registration flow: call backend, sync store, then hard-navigate to billing.
  // Hard navigation (window.location.href) forces a full page reload which
  // re-fires onAuthStateChanged with the user now in the DB — preventing the
  // race condition where currentOrg stays null after registration.
  async function finalizeRegistration(idToken: string) {
    await api.post("/api/v1/auth/register", {
      idToken,
      name: form.name.trim() || "Usuário",
      organizationName: form.organizationName.trim() || "Minha Empresa",
    });

    // Manually sync the auth store immediately so billing page has correct state
    try {
      const me = await api.get<MeResponse>("/api/v1/auth/me");
      const first = me.organizationUsers[0];
      if (first) {
        setCurrentOrg({
          id: first.organization.id,
          name: first.organization.name,
          slug: first.organization.slug,
          role: first.role,
          subscription: first.organization.subscription,
        });
      }
    } catch {
      // Non-critical — billing page waits for isInitialized before allowing checkout
    }

    // Full reload: ensures onAuthStateChanged re-fires and billing gate works
    window.location.href = "/billing";
  }

  // ── Email/password registration ────────────────────────────────────────────
  async function handleEmailRegister(e: React.FormEvent) {
    e.preventDefault();
    setIsLoading(true);
    setError(null);

    try {
      const credential = await createUserWithEmailAndPassword(
        auth,
        form.email,
        form.password
      );
      document.cookie = "firebase-session=1; path=/; max-age=3600; SameSite=Lax";
      setFirebaseUser(credential.user);
      const idToken = await credential.user.getIdToken();
      await finalizeRegistration(idToken);
      // If finalizeRegistration succeeds it calls window.location.href and
      // we never reach the lines below. Only errors fall through to catch.
    } catch (err) {
      const code = (err as { code?: string }).code ?? "";
      if (code === "auth/email-already-in-use") {
        setError("Este email já está em uso. Tente fazer login.");
      } else if (code === "auth/weak-password") {
        setError("A senha deve ter pelo menos 6 caracteres.");
      } else if (code === "auth/invalid-email") {
        setError("Email inválido.");
      } else if (code === "auth/network-request-failed") {
        setError("Erro de conexão. Verifique sua internet e tente novamente.");
      } else if (code === "auth/too-many-requests") {
        setError("Muitas tentativas. Aguarde alguns minutos e tente novamente.");
      } else {
        setError("Erro ao criar conta. Tente novamente.");
      }
      setIsLoading(false);
    }
  }

  // ── Complete registration for Google users already signed in ───────────────
  async function handleCompleteGoogleRegistration() {
    if (!auth?.currentUser) return;
    setIsLoading(true);
    setError(null);

    try {
      document.cookie = "firebase-session=1; path=/; max-age=3600; SameSite=Lax";
      const idToken = await auth.currentUser.getIdToken();
      await finalizeRegistration(idToken);
    } catch (err) {
      const code = (err as { code?: string }).code ?? "";
      if (code === "auth/network-request-failed") {
        setError("Erro de conexão. Verifique sua internet e tente novamente.");
      } else {
        setError("Falha ao completar cadastro. Tente novamente.");
      }
      setIsLoading(false);
    }
  }

  // ── Google sign-up (user not yet signed in) ────────────────────────────────
  async function handleGoogleRegister() {
    setIsLoading(true);
    setError(null);

    try {
      const credential = await signInWithPopup(auth, googleProvider);
      document.cookie = "firebase-session=1; path=/; max-age=3600; SameSite=Lax";
      setFirebaseUser(credential.user);
      if (!form.name && credential.user.displayName) {
        setForm((prev) => ({ ...prev, name: credential.user.displayName ?? "" }));
      }
      const idToken = await credential.user.getIdToken();
      await finalizeRegistration(idToken);
    } catch (err) {
      const code = (err as { code?: string }).code ?? "";
      if (code === "auth/popup-blocked") {
        setError("Popup bloqueado pelo navegador. Permita popups para este site e tente novamente.");
      } else if (code === "auth/popup-closed-by-user" || code === "auth/cancelled-popup-request") {
        setError("Login cancelado. Clique novamente para tentar.");
      } else if (code === "auth/network-request-failed") {
        setError("Erro de conexão. Verifique sua internet e tente novamente.");
      } else {
        setError("Falha ao entrar com Google. Tente novamente.");
      }
      setIsLoading(false);
    }
  }

  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8">
      <h2 className="text-xl font-bold text-white mb-6">Criar sua conta</h2>

      {googleSession ? (
        // ── Simplified form: Google user already authenticated, needs org setup ──
        <div className="flex flex-col gap-4">
          <div className="flex items-start gap-3 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 mb-1">
            <svg className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p className="text-sm text-blue-300">
              Google conectado como{" "}
              <span className="font-semibold text-blue-200">{form.email}</span>.
              Preencha os dados abaixo para concluir o cadastro.
            </p>
          </div>

          <Input
            id="name"
            label="Seu nome"
            placeholder="João Silva"
            value={form.name}
            onChange={setField("name")}
            required
          />

          <Input
            id="organizationName"
            label="Nome da empresa"
            placeholder="Minha Agência"
            value={form.organizationName}
            onChange={setField("organizationName")}
            required
          />

          {error && (
            <p className="text-sm text-red-400 bg-red-400/10 rounded-lg px-3 py-2 border border-red-400/20">
              {error}
            </p>
          )}

          <Button
            type="button"
            isLoading={isLoading}
            size="lg"
            className="w-full mt-1"
            onClick={handleCompleteGoogleRegistration}
            disabled={!form.name.trim() || !form.organizationName.trim()}
          >
            Completar cadastro
          </Button>
        </div>
      ) : (
        // ── Full form: new email/password user ─────────────────────────────────
        <>
          <form onSubmit={handleEmailRegister} className="flex flex-col gap-4">
            <Input
              id="name"
              label="Seu nome"
              placeholder="João Silva"
              value={form.name}
              onChange={setField("name")}
              required
              autoComplete="name"
            />

            <Input
              id="organizationName"
              label="Nome da empresa"
              placeholder="Minha Agência"
              value={form.organizationName}
              onChange={setField("organizationName")}
              required
            />

            <Input
              id="email"
              type="email"
              label="Email"
              placeholder="voce@empresa.com"
              value={form.email}
              onChange={setField("email")}
              required
              autoComplete="email"
            />

            <Input
              id="password"
              type="password"
              label="Senha"
              placeholder="Mínimo 6 caracteres"
              value={form.password}
              onChange={setField("password")}
              required
              autoComplete="new-password"
              minLength={6}
            />

            {error && (
              <p className="text-sm text-red-400 bg-red-400/10 rounded-lg px-3 py-2 border border-red-400/20">
                {error}
              </p>
            )}

            <Button type="submit" isLoading={isLoading} size="lg" className="w-full mt-1">
              Criar conta
            </Button>
          </form>

          <div className="relative my-5">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-white/10" />
            </div>
            <div className="relative flex justify-center text-xs text-gray-600">
              <span className="bg-[#0a0a0a] px-2">ou continue com</span>
            </div>
          </div>

          <Button
            type="button"
            variant="secondary"
            size="lg"
            className="w-full"
            onClick={handleGoogleRegister}
            disabled={isLoading}
          >
            <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
            </svg>
            Google
          </Button>
        </>
      )}

      <p className="mt-6 text-center text-sm text-gray-500">
        Já tem conta?{" "}
        <Link href="/login" className="text-brand-400 hover:text-brand-500 font-medium transition-colors">
          Entrar
        </Link>
      </p>
    </div>
  );
}
