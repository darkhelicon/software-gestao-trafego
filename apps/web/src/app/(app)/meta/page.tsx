...
export default function MetaPage() {
  ...
  // Refetch after OAuth redirect to show newly connected BMs
  useEffect(() => {
      ...
      {oauthError && (
        <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
          ...
          {oauthError === "invalid_state" &&
            "Sessão expirada. Tente conectar novamente."}
          {oauthError === "token_exchange" &&
            "Falha ao obter token. Tente novamente."}
          {oauthError === "no_business_manager" &&
            "Nenhum Business Manager encontrado na sua conta Meta."}
        </div>
      )}

      {searchParams.get("connected") === "true" && (
        <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">
          Business Manager(s) conectado(s) com sucesso! Clique em
          &ldquo;Sincronizar contas&rdquo; para importar suas contas de anúncios.
        </div>
      )}

      <section>
        <h2 className="text-base font-semibold text-gray-800 mb-4">
          Business Managers conectados
        </h2>
        {isLoading ? (
          ...
          <div className="rounded-xl border border-dashed border-gray-300 bg-white px-6 py-10 text-center">
            <p className="text-sm text-gray-500">
              Nenhum Business Manager conectado ainda.
            </p>
            <p className="text-xs text-gray-400 mt-1">
              Clique em &ldquo;Conectar Meta&rdquo; para começar.
            </p>
          </div>
        ) : (
