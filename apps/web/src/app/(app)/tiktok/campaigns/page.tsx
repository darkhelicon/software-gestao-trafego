    ...
    <div className="flex flex-col gap-8">
      ...
      {activeJobId && (
        <div className="rounded-xl border border-brand-400/20 bg-brand-400/5 p-6">
          <h2 className="text-sm font-semibold text-brand-400 mb-4">Criando campanha...</h2>
      ...
      {showForm && (
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-semibold text-white">Nova campanha TikTok</h2>
            <button
                ...
                {campaigns.items.map((c) => (
                  <tr key={c.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-400">
                      {(c.config as { objectiveType?: string } | null)?.objectiveType ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-gray-400">
                      {c.budget != null ? `R$ ${Number(c.budget).toFixed(2)}` : "—"}
                    </td>
